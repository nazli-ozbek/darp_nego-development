...
def _pickup_points(company_data: Dict, coordinates: Dict[str, List[float]]) -> np.ndarray:
    points: List[Tuple[float, float]] = []
    for client in company_data.get("clients", []):
        loc_id = str(client.get("start_location"))
        if loc_id in coordinates:
            x, y = coordinates[loc_id]
            points.append((x, y))
    if not points:
...
def compute_company_metrics(
    ...
    return {
        ...
        "request_count": int(points.shape[0]),
