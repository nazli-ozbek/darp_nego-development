...
def analyze_agent_costs(log_dirs):
    ...
    for log_dir in log_dirs:
        ...
        try:
            ...
            cost_reduction = 100 * (
                        total_initial_cost - total_final_cost) / total_initial_cost if total_initial_cost > 0 else 0
...
def get_log_directories(log_root_name: str = "logs"):
    """
...
if __name__ == "__main__":
    ...
    for log_root_name in log_roots:
        print(f"\n=== Analyzing log root: {log_root_name} ===")
        log_dirs = get_log_directories(log_root_name=log_root_name)

        if not log_dirs:
            print("No log directories found matching the specified patterns!")
            continue

        # Derive dataset name and output directory under stats/<dataset>_analyze_<log_root>
        ...
        output_dir = os.path.join(base_dir, "stats", f"{dataset_name}_analyze_{log_root_name}")
        os.makedirs(output_dir, exist_ok=True)

        # Diagnostic: report presence of routing/negotiation JSONs per session
        report_lines = [
            f"Dataset: {dataset_name}",
            f"Log root: {log_root_name}",
            f"Logs scanned: {len(log_dirs)}",
            "",
        ]
        missing_routing = 0
        ...
        for log_dir in log_dirs:
            ...
            report_lines.append(
                f"{session} | routing: {'OK' if has_routing else 'MISSING'} | negotiation: {'OK' if has_neg else 'MISSING'}"
            )
        report_lines.append("")
