...
def calculate_summary_statistics(df):
    ...
    summary_stats = {
        ...
        'overall_transfer_acceptance_rate': (df['total_accepted_transfers'].sum() / df['total_proposed_transfers'].sum() * 100) if df['total_proposed_transfers'].sum() > 0 else 0
    }
...
def create_visualizations(df, rounds_df, output_dir="analysis", dataset_name=None):
    ...
    plt.pie(agreement_counts.values, labels=['No Cost Change' if not x else 'Cost Change Detected' for x in agreement_counts.index], 
    ...
    plt.pie(last_round_counts.values, labels=['Last Round Rejected' if not x else 'Last Round Accepted' for x in last_round_counts.index], 
            autopct='%1.1f%%', startangle=90, colors=colors)
    ...
    summary_data = [
        ...
        ['Cases with Last Round Accepted', f"{summary_stats['cases_with_last_round_accepted']} ({summary_stats['last_round_acceptance_rate']:.1f}%)"],
    ...
    plt.title(f'{dataset_name} Dataset Analysis Summary' if dataset_name else 'Dataset Analysis Summary', fontsize=16, fontweight='bold', pad=20)
    plt.tight_layout()
...
def print_detailed_analysis(df, rounds_df, dataset_name=None):
    ...
    print(f"   Cases with cost change: {summary_stats['cases_with_agreement']} ({summary_stats['agreement_rate']:.1f}%)")
    print(f"   Cases with last round accepted: {summary_stats['cases_with_last_round_accepted']} ({summary_stats['last_round_acceptance_rate']:.1f}%)")
...
def main():
    ...
    try:
        ...
        dataset_name, session_folders = find_latest_dataset_logs()
        
        if not session_folders:
            print("❌ No log directories found matching the pattern!")
            return
        
        # Output directory under python/darp_nego/stats/<dataset_name>
        base_dir = os.path.abspath(os.path.dirname(__file__))
        output_dir = os.path.join(base_dir, "stats", dataset_name or "analysis")
        ...
        print(f"\n📁 Generated files:")
        print(f"   - negotiation_analysis.csv (detailed case data)")
        print(f"   - rounds_analysis.csv (round-by-round data)")
        print(f"   - summary_statistics.csv (summary metrics)")
        print(f"   - agreement_success_rate.png")
        print(f"   - last_round_acceptance_rate.png")
        print(f"   - cost_comparison.png")
        print(f"   - cost_change_vs_agents.png")
        print(f"   - negotiation_rounds_distribution.png")
        print(f"   - execution_time_vs_agents.png")
        print(f"   - full_approval_rates.png")
        print(f"   - round_dynamics.png")
