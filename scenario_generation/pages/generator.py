...
def distribute_to_companies(vehicles, clients, num_companies, coordinates, num_vehicles, num_hospitals):
    ...
    # Assign clients to companies and update their start locations' coordinates
    for client in clients:
        h_id = client["client_id"] % num_companies  # Assign to company based on modulo
        companies[f"company_{h_id}"]["clients"].append(client)
        
        # Update coordinates for this client's start location to be in the correct quadrant
        quadrant = h_id % 4  # We'll cycle through 4 quadrants
        coordinates[client["start_location"]] = generate_quadrant_coordinate(quadrant)
    
    return companies

# Streamlit UI
st.title("DARP Case Generator with Coordinates (-10 to 10 range)")
...
if st.button("Generate Cases"):
    ...
    for case_index in range(1, num_cases + 1):
        ...
        companies = distribute_to_companies(vehicles, clients, num_companies, coordinates, num_vehicles, num_hospitals)
        
        # Generate the time matrix with all coordinates
