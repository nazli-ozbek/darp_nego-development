...
def generate_clients_with_time_window_outliers(num_clients, num_vehicles, num_hospitals, min_vehicle_capacity=5):
    """
    Generate clients where each company's clients have similar characteristics,
    but one client per company will have almost-overlapping time windows.
    """
    clients = []
...
def distribute_to_companies_with_time_window_outliers(vehicles, clients, num_companies, coordinates, num_vehicles, num_hospitals):
    """
    Distribute clients to companies ensuring each company has one client with almost-overlapping time windows.
    All clients within a company have similar data characteristics, but one has tight time constraints.
    """
    ...
    for company_id in range(num_companies):
        ...
        # Modify one client to have almost-overlapping time windows
        if company_clients:
            outlier_client_index = random.randint(0, len(company_clients) - 1)
            outlier_client = company_clients[outlier_client_index]
            
            # Create almost-overlapping time windows for the outlier
            early_pickup = outlier_client['early_pickup']
            
            # Very tight pickup window (almost overlapping)
            pickup_window = random.randint(1, 3)  # Very small window
            late_pickup = early_pickup + pickup_window
            
            # Very tight drop window (almost overlapping)
            travel_time = random.randint(5, 15)  # Short travel time
            early_drop = late_pickup + travel_time
            drop_window = random.randint(1, 3)  # Very small window
            late_drop = early_drop + drop_window
            
            # Update the outlier client
            outlier_client['late_pickup'] = late_pickup
            outlier_client['early_drop_off'] = early_drop
            outlier_client['late_drop_off'] = late_drop
...
def generate_scenarios_with_time_window_outliers():
    """
    Generate 10 scenarios with time window outliers:
    ...
    but one client per company has almost-overlapping time windows.
    """
    scenarios = []
    ...
    for i, config in enumerate(scenario_configs):
        ...
        clients = generate_clients_with_time_window_outliers(num_clients, num_vehicles, num_hospitals)
        vehicles = generate_vehicles(num_vehicles)
        
        # Distribute to companies with time window outlier positioning
        companies = distribute_to_companies_with_time_window_outliers(vehicles, clients, num_companies, coordinates, num_vehicles, num_hospitals)
...
if __name__ == "__main__":
    # Generate scenarios
    scenarios = generate_scenarios_with_time_window_outliers()
    
    # Create timestamp folder
    timestamp = "2025_08_08_14_00_time_windows"
    ...
    print(f"Generated {len(scenarios)} scenarios with time window outliers!")
    ...
    print(f"\nTIME WINDOW OUTLIER FEATURES:")
    print(f"- Coordinates stay within -10 to +10 bounds")
    print(f"- Consistent speed of 0.5 units/minute for all scenarios")
    print(f"- Time matrix calculated from coordinates")
    print(f"- Each company has clients with similar data characteristics")
    print(f"- Each company has one client with almost-overlapping time windows")
    print(f"- Pickup windows: 1-3 minutes (very tight)")
    print(f"- Drop windows: 1-3 minutes (very tight)")
    print(f"- This should help the farthest distance algorithm find solutions in earlier rounds") 