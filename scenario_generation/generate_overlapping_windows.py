#!/usr/bin/env python3
"""
Standalone DARP Case Generator with Overlapping Time Windows

This script generates DARP scenarios ensuring that at least 2 clients per company 
have overlapping time windows, creating more realistic negotiation scenarios.
"""
...
def generate_cases_with_overlapping_windows(num_cases=50, client_range=(10, 15), vehicle_range=(7, 8), 
                                          hospital_range=(2, 10), company_range=(2, 4), seed=None):
    """
    Generate multiple DARP cases with overlapping time windows.
    
    Args:
        num_cases: Number of cases to generate
        client_range: Tuple of (min, max) clients per case
        vehicle_range: Tuple of (min, max) vehicles per case
        hospital_range: Tuple of (min, max) hospitals per case
        company_range: Tuple of (min, max) companies per case
        seed: Random seed for reproducibility
    
    Returns:
        tuple: (cases_data, output_folder, verification_summary)
    """
    if seed is not None:
        random.seed(seed)
    ...
    verification_summary = {
        ...
        "cases_with_all_overlaps": 0,
        "cases_with_some_overlaps": 0,
        "cases_with_no_overlaps": 0,
        "overlap_details": []
    }
    
    print(f"Generating {num_cases} cases with overlapping time windows...")
    
    for case_index in range(1, num_cases + 1):
        print(f"  Generating case {case_index}/{num_cases}...")
        ...
        some_have_overlap = any(result["has_overlap"] for result in verification.values())
        
        if all_have_overlap:
            verification_summary["cases_with_all_overlaps"] += 1
        elif some_have_overlap:
            verification_summary["cases_with_some_overlaps"] += 1
        else:
            verification_summary["cases_with_no_overlaps"] += 1
        
        verification_summary["overlap_details"].append({
            "case": case_index,
            "verification": verification,
            "all_have_overlap": all_have_overlap
        })
    ...
    print(f"Generation complete! Data saved to: {file_path}")
    
    return cases_data, folder_name, verification_summary

def print_verification_summary(verification_summary):
    """Print a summary of the verification results."""
    print("\n" + "="*60)
    print("OVERLAPPING TIME WINDOWS VERIFICATION SUMMARY")
    print("="*60)
    print(f"Total cases generated: {verification_summary['total_cases']}")
    print(f"Cases where ALL companies have overlaps: {verification_summary['cases_with_all_overlaps']}")
    print(f"Cases where SOME companies have overlaps: {verification_summary['cases_with_some_overlaps']}")
    print(f"Cases where NO companies have overlaps: {verification_summary['cases_with_no_overlaps']}")
    
    success_rate = (verification_summary['cases_with_all_overlaps'] / verification_summary['total_cases']) * 100
    print(f"Success rate (all companies with overlaps): {success_rate:.1f}%")
    
    if success_rate < 100:
        print("⚠️  WARNING: Not all cases have complete overlaps!")
        print("   This should be 100% with the fixed algorithm.")
    
    # Show details for cases without full overlap
    problematic_cases = [detail for detail in verification_summary['overlap_details'] 
                        if not detail['all_have_overlap']]
    
    if problematic_cases:
        print(f"\nCases with incomplete overlaps ({len(problematic_cases)}):")
        for detail in problematic_cases[:5]:  # Show first 5
            case_num = detail['case']
            verification = detail['verification']
            companies_without_overlap = [company for company, result in verification.items() 
                                       if not result['has_overlap']]
            print(f"  Case {case_num}: Companies without overlaps - {companies_without_overlap}")
    else:
        print("✅ All cases have complete overlaps - Perfect!")

if __name__ == "__main__":
    # Example usage
    print("DARP Case Generator with Overlapping Time Windows")
    print("=" * 50)
    
    # Generate cases
    cases_data, output_folder, verification_summary = generate_cases_with_overlapping_windows(
        num_cases=10,  # Generate 10 cases for testing
        client_range=(10, 15),
        vehicle_range=(7, 8),
        hospital_range=(2, 10),
        company_range=(2, 4),
    ...
    print_verification_summary(verification_summary)
    
    print(f"\nGenerated data saved to: {output_folder}/")
    print("You can now use this data with your DARP negotiation system!") 