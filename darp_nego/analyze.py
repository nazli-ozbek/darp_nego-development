...
def analyze_agent_costs(log_dirs):
    ...
    for log_dir in log_dirs:
        # Extract directory name and check if it contains agent count
        dir_name = os.path.basename(log_dir)
        agent_count_match = re.search(r'(\d+)agents', dir_name)
...
def get_log_directories():
    """
    Get all log directories with specific patterns for each agent count.
    
    Returns:
        list: List of log directory paths
    """
    folder_patterns = {
        '3 agents': "logs/2025_05_05_11_17_3agents_case_*",
        '4 agents': "logs/2025_05_05_*_4agents_case_*",
        '5 agents': "logs/2025_05_05_17_00_5agents_case_*",
        '6 agents': "logs/2025_05_05_20_49_6agents_case_*"
    }
    
    all_directories = []
    agent_counts = {}
    
    for label, pattern in folder_patterns.items():
        directories = glob.glob(pattern)
        case_count = min(len(directories), 50)  # Take up to 50 cases for each agent count
        directories = directories[:case_count]
        
        if directories:
            print(f"Found {len(directories)} directories for {label}")
            all_directories.extend(directories)
            
            # Extract agent count for statistics
            agent_count_match = re.search(r'(\d+)agents', label)
            if agent_count_match:
                agent_count = int(agent_count_match.group(1))
                agent_counts[agent_count] = len(directories)
...
if __name__ == "__main__":
    ...
    if df_negotiation.empty:
        ...
        table2.to_csv("stats/table2_expanded_results.csv", 
                      float_format="%.2f", index=False)
        print("\nTable saved to stats/table2_expanded_results.csv")
    ...
    if df_cost_data.empty:
        ...
        create_cost_boxplot(df_cost_data)
    ...
    if not df_cost_data.empty:
        create_cost_line_plot(df_cost_data)
    ...
    create_agent_scenario_table(log_dirs)
